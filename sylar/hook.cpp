...
static _HookIniter s_hook_initer; // 静态对象, 这表示在main函数执行之前就会获取各个符号的地址并保存在全局变量中
bool is_hook_enable() {
...
unsigned int sleep(unsigned int seconds) {
	if (!sylar::t_hook_enable) {
		SYLAR_LOG_DEBUG(g_logger) << "当前线程未开启hook, 使用系统sleep";
		return sleep_f(seconds);
	}
	SYLAR_LOG_DEBUG(g_logger) << "协程任务使用hook sleep(), 添加" << seconds << "秒定时器";
...
int usleep(useconds_t usec) {
	...
	sylar::IOManager* iom = sylar::IOManager::GetThis();
    /*
    (void(sylar::Scheduler::*)(sylar::Fiber::ptr, int thread))&sylar::IOManager::schedule是一个类型转换
    将 sylar::IOManager::schedule 函数指针转换为指向 sylar::Scheduler 类的成员函数指针类型 (void(sylar::Scheduler::*)(sylar::Fiber::ptr, int thread))
    */
