...
sylar::ConfigVar<float>::ptr g_int_valuex_config =
    sylar::Config::Lookup("system.port", (float)8080.0, "system port");
...
std::map<YAML::NodeType::value, std::string> nodeTypeNames {
...
void print_yaml(const YAML::Node& node, int level) {
    if(node.IsScalar()) {
        SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level * 4, ' ')
            << node.Scalar() << "-" << nodeTypeNames.at(node.Type()) << "-" << level;
    } else if(node.IsNull()) {
        SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level * 4, ' ')
            << "NULL-" << nodeTypeNames.at(node.Type()) << "-" << level;
    } else if(node.IsMap()) {
        std::cout << "------------" << std::endl;
        for(auto it = node.begin(); it != node.end(); ++it) {
            SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level * 4, ' ')
                    << it->first << "-" << nodeTypeNames.at(it->second.Type()) << "-" << level;
            print_yaml(it->second, level + 1);
        }
    } else if(node.IsSequence()) {
        for(size_t i = 0; i < node.size(); ++i) {
            SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << std::string(level * 4, ' ')
                << i << "-" << nodeTypeNames.at(node[i].Type()) << "-" << level;
            print_yaml(node[i], level + 1);
        }
    }
}

// void print_yaml(const YAML::Node& node, const std::string& prefix = "") {
//   switch (node.Type()) {
//     case YAML::NodeType::Null:
//       SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << prefix << " = null\n";
//       break;
//     case YAML::NodeType::Scalar:
//       SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << prefix << " = " << node.as<std::string>() << "\n";
//       break;
//     case YAML::NodeType::Sequence:
//       for (std::size_t i = 0; i < node.size(); ++i) {
//         print_yaml(node[i], prefix + "[" + std::to_string(i) + "]");
//       }
//       break;
//     case YAML::NodeType::Map:
//       for (auto it = node.begin(); it != node.end(); ++it) {
//         print_yaml(it->second, prefix + "[" + it->first.as<std::string>() + "]");
//       }
//       break;
//     case YAML::NodeType::Undefined:
//       SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << prefix << " = undefined\n";
//       break;
//   }
// }
